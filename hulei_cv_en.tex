...
\cvlistitem{Proficient experience on \textit{GCC}, and \textit{GNU binutils}, especially \textit{VMware} toolchain}
\cvlistitem{Sound knowledge on \textit{GCC} binary code layout, as well as the \textit{ELF} format}
\cvlistitem{Strong userland/kernel debugging and troubleshooting skills, especially on \textit{ESX/VMKernel}}
%\cvlistitem{Skillful at \textit{C++/STL}, familiar with \textit{C++ Standards}}
\cvlistitem{Good at \textit{SCons} and \textit{Makefiles}, especially familiar with \textit{VMware}'s build environment}
...
\cvtitlelistitem{Environment}{3.5 years \textit{ESX} experience, 2+ years vmkernel experience}
...
\cvlistitem{Understand software testing processes well, especially \textit{VMware} \textit{ESX} release model}
